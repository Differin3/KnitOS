// SHA-256 + HMAC-SHA256 (FIPS 180-4 / RFC 2104). Freestanding, no libc.
#include "sha256.h"
#include <string.h>
...
static void sha256_transform(struct sha256_ctx* ctx, const uint8_t* d) {
    uint32_t m[64];
    for (int i = 0; i < 16; i++)
    ...
    uint32_t a=ctx->state[0], b=ctx->state[1], c=ctx->state[2], dd=ctx->state[3];

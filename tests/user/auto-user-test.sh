#!/usr/bin/env bash
# Standalone guest user-space autotest (QEMU -> serial), mirrors tests/fs/auto-fs-test.sh
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
cd "$ROOT"

AUTO_LOG="$ROOT/logs/auto-user-test.log"
SERIAL_LOG="$ROOT/logs/qemu-serial.log"
SERIAL_PORT="${SERIAL_PORT:-4445}"
BOOT_TIMEOUT="${BOOT_TIMEOUT:-90}"
USER_TIMEOUT="${USER_TIMEOUT:-180}"
...
echo "== Starting QEMU for user autotest"
qemu-system-i386 \
    -cdrom "$ROOT/myos.iso" \
    -m 128 \
    ...
    "${QEMU_DISPLAY[@]}" \
    -no-reboot \
    >"$ROOT/logs/qemu-user-stdout.log" 2>&1 &
QEMU_PID=$!
...
python3 - "$SERIAL_PORT" "$BOOT_TIMEOUT" "$USER_TIMEOUT" "$SERIAL_LOG" <<'PY'
import os, socket, sys, threading, time

PORT = int(sys.argv[1])
BOOT_TIMEOUT = float(sys.argv[2])
USER_TIMEOUT = float(sys.argv[3])
...
print("== Sending: autotest user")
sock.sendall(b"autotest user\n")

if not wait_substrings(["[AUTOTEST] user ok", "[INF][autotest] user_ok"], USER_TIMEOUT,
                       progress_label="waiting user_ok"):
    text = get_text()
    print("[FAIL] user autotest did not pass")
    print("--- serial tail ---")
    print(text[-3000:])
    sys.exit(1)

print("[PASS] guest user-space autotest")
stop.set()
sock.close()
sys.exit(0)
PY
...
RC=$?
if [[ "$RC" -ne 0 ]]; then
    fail "user orchestrator failed ($RC)"
fi

for m in isolev_ok demo2_ok demo3_killed_ok user_ok; do
...
if grep -qE 'isolev_kernel_user|isolev_mark_failed|demo2_spawn_failed|demo2_timeout|demo2_output_mismatch|demo3_spawn_failed|demo3_survived' "$SERIAL_LOG" 2>/dev/null; then
    fail "user autotest reported failure"
fi

echo ""
echo "============================================"
echo " USER-SPACE AUTO TEST PASSED"
echo " Logs: $AUTO_LOG"
echo "       $SERIAL_LOG"
echo "============================================"
exit 0